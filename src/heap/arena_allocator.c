...
arena_t* init(size_t size) {
  ...
  // addr NULL: Don't care where the mapping starts
  // prot PROT_READ, PROT_WRITE: Pages can be read and written
  // flags MAP_PRIVATE: Changes are private to the process
  // flags MAP_ANONYMOUS: Mapping not backed by a file
